...
ThumbAI exposes a safe storage configuration check at:

```text
GET /api/storage/health
```

The response only reports whether required variables are present. It does not return access keys, secret keys, bucket values, or endpoint values.

Use this before testing homepage reference-image uploads or real thumbnail generation. Storage is ready when `ready` is `true` and `missingRequired` is empty.

## Cloudflare R2 Setup For ThumbAI

Use R2 for generated thumbnails, homepage reference images, workbench assets, and future user uploads.

1. Create a Cloudflare R2 bucket for ThumbAI, for example `thumbai-assets`.
2. Create an R2 API token with object read/write access for that bucket.
3. Copy `env.example` to `.env.local` if you do not already have one.
4. Fill the storage variables:

```env
STORAGE_REGION=auto
STORAGE_BUCKET_NAME=thumbai-assets
STORAGE_ACCESS_KEY_ID=your-r2-access-key-id
STORAGE_SECRET_ACCESS_KEY=your-r2-secret-access-key
STORAGE_ENDPOINT=https://your-account-id.r2.cloudflarestorage.com
STORAGE_PUBLIC_URL=https://cdn.thumbai.com
```

`STORAGE_PUBLIC_URL` is optional during local development, but recommended before launch so saved thumbnail URLs are stable and brandable.

After changing `.env.local`, restart the dev server and open:

```text
http://127.0.0.1:3001/api/storage/health
```

The setup is ready when the response says `ready: true`. Then test the homepage reference-image upload with a small `jpg`, `png`, or `webp` file under `4MB`.
